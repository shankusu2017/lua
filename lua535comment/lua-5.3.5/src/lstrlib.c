...
/* 
** 计算字符串的长度，待计算的字符串存放在函数第一个参数中，也就是紧跟在
** 栈中的函数指针的后面的那个参数
*/
static int str_len (lua_State *L) {
  size_t l;
  /* luaL_checklstring()通过l传出了字符串长度，然后调用lua_pushinteger()将长度压栈 */
...
/* 将在栈中紧跟在str_lower函数指针后面的那个参数转换为其小写形式 */
...
/* 将在栈中紧跟在str_lower函数指针后面的那个参数转换为其大写形式 */
