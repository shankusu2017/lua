...
/*
** sizeof(l)/sizeof((l)[0])这个表达式计算的是数组l的元素个数
** luaL_newlibtable用于创建一个table，其中table的数组部分大小为0，hash表部分
** 的大小和数组l的元素个数一样。从这里可以看出，库对应的函数列表存放在
** table的hash表部分。这个table也是会压入堆栈的。
*/
#define luaL_newlibtable(L,l)	\
  lua_createtable(L, 0, sizeof(l)/sizeof((l)[0]) - 1)

/*
** luaL_newlib()这个宏主要做了以下三件事情：
...
/*
** luaL_fofile()用于执行lua文件，其中luaL_loadfile()用于进行语法和
** 词法分析生成字节码，然后调用lua_pcall()用于执行前面生成的字节码。
*/
...
/* 
** lua中的buffer对象，包含了buffer的地址、长度以及buff中的字符数量，
** 以及全局的状态信息
*/
