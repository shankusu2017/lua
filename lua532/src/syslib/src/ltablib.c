...
static int maxn (lua_State *L) {
  ...
  while (lua_next(L, 1)) {
    lua_pop(L, 1);  /* remove value reserve key */
...
//仔细读下面的函数，有意思哦
//将给定的lst依次加入新构建的tbl[1,2,...N]中，并且设置n域
static int pack (lua_State *L) {
  ...
  lua_insert(L, 1);         /* put it(new-table) at index 1 */
...
/* size of 'e' measured(基于) in number of 'unsigned int's */
