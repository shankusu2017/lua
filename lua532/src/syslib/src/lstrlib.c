...
** pattern-matching. This limit is arbitrary(随意的).
*/
...
/* 计算字符串的长度，若不是字符串，尝试转换成字符串，转换失败则报错 */
...
/* 
 *translate a relative(相对的) string position: negative(负的) means back from end 
 * 负索引则修正到合理的位置，
 * 正索引，直接返回，所以调用者还需要自己判断一次
*/
static lua_Integer posrelat (lua_Integer pos, size_t len) {
  if (pos >= 0) return pos;
  else if (0u - (size_t)pos > len) return 0;	/* 转换后的实际效果是 if (abs(pos) > len return 0) */
  else return (lua_Integer)len + pos + 1;		/* 这里可以看到 -1：是指代最后一个字符 */
}
...
static int str_sub (lua_State *L) {
  ...
  lua_Integer end = posrelat(luaL_optinteger(L, 3, -1), l);	/* 第三个参数没有则默认为-1 */
	
  if (start < 1) start = 1;						/* 这里是必须要考虑的，因为valid.idx[1,len],下同 */
	
  if (end > (lua_Integer)l) end = l;	
	
  if (start <= end)
    lua_pushlstring(L, s + start - 1, (size_t)(end - start) + 1);
  else lua_pushliteral(L, "");			 	/* 哈哈，这里直接返回了 "" */
  return 1;
}
...
/* 接收零或更多的整数。 返回和参数数量相同长度的字符串。 其中每个字符的内部编码值等于对应的参
数值。 */
...
//给所有的string类指定metaTable
static void createmetatable (lua_State *L) {
  // create metaTable
  lua_createtable(L, 0, 1);  /* table to be metatable for strings */

  // set metaTanble for string
