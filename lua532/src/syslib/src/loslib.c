...
static int os_date (lua_State *L) {
  ...
  if (*s == '!') {  /* UTC? */
		/*
		 * 将日历时间转换为UTC(0时区)表示的year,mon,day,hour,min,sec 
		 * gm:是Greenwich Mean Time的缩写(格林威治-英国伦敦-0时区)
	  */
    stm = l_gmtime(&t, &tmr);  
    s++;  /* skip '!' */
  }
  else
    stm = l_localtime(&t, &tmr); // 将日历时间转换为配置所在的时区(本地时区)的year,mon,day,hour,min,sec
