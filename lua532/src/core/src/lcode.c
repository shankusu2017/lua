...
/* 生成一条LOADNIL指令
** 如果前面已有LOADNIL则尝试衔接，以便减少指令数(算是一种优化吧)
*/
void luaK_nil (FuncState *fs, int from, int n) {
  Instruction *previous;
  int l = from + n - 1;  /* last register to set nil */
  
  /* 如果前一个指令是loadNil，那么这里尝试将本次生成的loadNil和前面的连接起来 */
...
** optimizations(优化) with consecutive instructions not in the same basic block).
*/
...
/*
** 不太明白干啥？用于重定位语句标签的相关逻辑?
** 因为Lua语法分析是一遍生成的，所以goto语句生成时，对应的lable可能还没有
** 可能这里是看看有没有被悬挂的goto标签要定位的？
*/
...
void luaK_patchlist (FuncState *fs, int list, int target) {
  ...
  /* goto 在前， label在后，且是此行(往前跳)
  
  ** goto lbl
  ** ::lbl:: print("i'm lbl line...")
  */
  if (target == fs->pc) {	
...
int luaK_codek (FuncState *fs, int reg, int k) {
  if (k <= MAXARG_Bx) { /* 一条指令可以搞定 */
    return luaK_codeABx(fs, OP_LOADK, reg, k);
  } else {              /* 参数太大了，需要两条指令才能搞定 */
...
**
** KEYFUNCTION 关键函数:处理常量
** 可以看到函数中的常量被缓存起来了，且保持唯一一份(避免冗余)
*/
static int addk (FuncState *fs, TValue *key, TValue *v) {
  ...
  if (ttisinteger(idx)) {  /* is there an index there? */
    /* 按照结果列表里面给出的指引值去f->k中核对下，确认是否为本次要找的k */
    k = cast_int(ivalue(idx));
    /* correct value? (warning: must distinguish(区分) floats from integers!) */
    if (k < fs->nk && ttype(&f->k[k]) == ttype(v) && luaV_rawequalobj(&f->k[k], v))
      return k;  /* reuse index */
  }

  /* 运行到这里，意味着上述缓存结果列表被击穿了或尚未缓存相关结果
  ** 这里将v压入Proto->k数组并更新上述缓存
  */
...
void luaK_dischargevars (FuncState *fs, expdesc *e) {
  switch (e->k) {
    ...
    case VVARARG:
    case VCALL: { 
      /* local a,b,c,e = 10, fun(), 20
      ** 上面这种情况时，仅接收fun()的一个返回值
      */
      luaK_setoneret(fs, e);
      break;
    }
...
static void exp2reg (FuncState *fs, expdesc *e, int reg) 
{
  ...
  /* 生成新的指令，尝试重定位已经悬挂未决的的语句(eg:goto,brak?)  */
  if (hasjumps(e)) {
...
void luaK_exp2nextreg (FuncState *fs, expdesc *e) {
  luaK_dischargevars(fs, e);    /* discharge:卸载 */
  freeexp(fs, e);
  luaK_reserveregs(fs, 1);
...
/*
** Try to "constant-fold" an operation; return 1 iff successful。 fold(折叠)
** 
** 适用于下列场景？ local a = 1+2---->简化成 a = 3?
*/
...
/*
** Code for binary and unary(一元) expressions that "produce values"
