...
#define NAME_BUF_MAX 10240
#define CONTENT_BUF_MAX (1024*1024)
...
static int sendM(lua_State *L)
{
    ...
    if (n <= 0) {
        perror("sendM arg.cnt is invalid\n");
        return 0;
    }
    int type = lua_type(L, 1); //仅仅查看第一个参数的类型是否是表
    if (LUA_TTABLE != type) {
        perror("arg.type must be table, not ");
        char buf[1024] = {0};
        sprintf(buf, "%s", lua_typename(type));
        perror(buf);
        return 0;
    }
    ...
    //只保留第一个参数，其它的统统的丢弃
    lua_settop(L, 1);
    int retType = lua_getfield(L, 1, "name");
    if (LUA_TSTRING != retType) {
        perror("mail.recvName.type must be string\n");
        return 0;
    }
    char name[NAME_BUF_MAX] = {0};
    size_t len = 0;
    char *addr = lua_tolstring(L, 1, &len);
    strncpy(name, addr, sizeof(name) - 1);
    printf("mail.recvName:%s\n", name);

    //弹掉上面的key-value
    lua_pop(L, 2);
    retType = lua_getfield(L, 1, "content");
    if (LUA_TSTRING != retType) {
        perror("mail.content.type must be string\n");
        return 0;
    }
    char content[CONTENT_BUF_MAX] = {0};
    len = 0;
    addr = lua_tolstring(L, 1, &len);
    strncpy(content, addr, sizeof(content) - 1);
...
//这里的函数名和生存的库名必须"兼容"
extern "C" __declspec(dllexport) int luaopen_tfmail(lua_State* L)
{
    luaL_register(L,"tfmail", mailLib); //注册新的模块名
    return 1;
}
