...
** arg.1: fileName
** arg.2: 若为modeName,则返回对应的数值，
**          若为tbl，则将所有参数的值填入表中，若不是tbl则构建一张表，且将所有参数的值填入表中
*/
...
static int push_link_target(lua_State *L)
{
        ...
        while (1) {
            ...
            if (!target) /* failed to allocate */
                return 0;
            tsize = readlink(file, target, size); /* 返回字符串的真实字符数 */
            if (tsize < 0) { /* a readlink() error occurred */
                free(target);
                return 0;
            }
            if (tsize < size) /* tsize表示上述readlink中的真实字符串的长度(不包含\0)，可能比size大 */
                break;
...
** 根据传入的mode.key求mode.val
*/
static int link_info (lua_State *L)
{
    int ret;
    /* 获取连接的对象的绝对路径 mode.key=target,求mode.val */
    if (lua_isstring (L, 2) && (strcmp(lua_tostring(L, 2), "target") == 0)) {
      int ok = push_link_target(L);
      return ok ? 1 : pusherror(L, "could not obtain（获得） link target");
    }

    /*
     * 传入了一个其它的mode.key=x,求对应的mode.val
     * 若传入了tbl，则提取所有的值域，
     * 若没有传入tbl，则构建一张tbl,且填入所有的值域并返回这张表
     */
